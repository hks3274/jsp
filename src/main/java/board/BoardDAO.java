			...
			if(search == null || search.equals("")) {
				...
				if(contentsShow.equals("adminOK")) {
				  ...
				  pstmt.setString(1, "%"+searchString+"%");
					...
					sql = "select sum(a.cnt) as cnt from (select count(*) as cnt from board where openSW = 'OK' and complaint = 'NO' and  "+search+" like ? union select count(*) as cnt from board where mid = ? and  "+search+" like ? and (openSW = 'NO' or complaint = 'OK')) as a";
					pstmt = conn.prepareStatement(sql);
					pstmt.setString(1, "%"+searchString+"%");
					pstmt.setString(2, contentsShow);
					pstmt.setString(3, "%"+searchString+"%");
