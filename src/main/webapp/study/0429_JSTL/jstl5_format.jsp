...
<div class = "container">
  ...
  <div class="text-right">
    ...
    <button type="button" onclick="javascript:location.href='jstl2.jsp';" class="btn btn-info btn-sm">JSTL반복문</button>
    <button type ="button" onclick = "javascript:location.href='jstl3_vo.jsp';" class="btn btn-primary btn-sm">JSTL 반복문응용</button>
		<button type ="button" onclick = "javascript:location.href='jstl4_function.jsp';" class="btn btn-secondary btn-sm">JSTL function</button>
  </div>
  <hr/>
	...
	<div>
		...
		<fmt:formatNumber value ="${won1}" pattern="#,##0.0" /> : <fmt:formatNumber value ="${won2}"  pattern="#,##0.0"/><br/> <!-- 서식기호 '#'은 무효의(의미가 없는) 0을 표시하지 않는다. -->
		...
		백분율1 : <fmt:formatNumber value="${per}" type="percent" pattern="0.0%" /><br/>
		...
		현재날짜시간3 : <fmt:formatDate value="${today}" pattern="yyyy년 MM월 dd일 hh시 mm분 ss초"/><br/>
	 	<hr/>
