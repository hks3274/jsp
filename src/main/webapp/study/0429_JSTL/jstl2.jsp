...
<div class = "container">
	...
	<div class = "text-right">
		<button type ="button" onclick = "javascript:location.href='jstl1.jsp';" class="btn btn-success btn-sm">JSTL core라이브러리</button>
		<button type ="button" onclick = "javascript:location.href='jstl3_vo.jsp';" class="btn btn-info btn-sm">JSTL 반복문응용</button>
		<button type ="button" onclick = "javascript:location.href='jstl4_function.jsp';" class="btn btn-primary btn-sm">JSTL 함수</button>
	...
	<pre>
		사용법1 < c : forEach var ="변수" begin="초기값" end = "최종값" step ="증감값" varStatus="매개변수"></ c : forEach>
	 					:step이 1인 경우는 생략가능하다. varStatus="변수"는 필요할때만 선언해서 사용하면 된다.
	 	사용법2 : < c : forEach var="변수" item="$ {객체명/배열}" varStatus="매개변수" > ~~ </ c : forEach>
	 	사용법3 : < c : forTokens var="변수" item="$ {객체명/배열}" delims="구분기호" > ~~ </ c : forTokens>
...
%>
	...
	*${i}단*<br/>
	...
	<br/>
	<h5>4.배열의 그림명을 저장된 그림을 출력하시오.(단, 한줄에는 3개의 그림을 출력하시오.)</h5>
	<div>
...
%>
	...
	<hr/>
<h5> 6. 5번열에서 '서울'단어에는 빨간 색으로 표시해서 출력하시오.(배열에 저장된 자료를 한행에 3개의 항목씩 출력하시오.)</h5>	
	...
	<h5> 7. 6번 문제 조건에 추가 : 나이가 25세 미만은 굵고 파랑색 표시</h5>	

	<c:forEach var="member" items="${members}">
		<c:forEach var="m" items="${member}">
			<c:if test="${m < '25'}"><font color="blue">${m}</font></c:if>
	...
	<br/>
	<h5>8.앞의 사용법2번에 저장한 cards의 내용 중, ' 첫번째 카드의 배경색은 노랑 마지막카드의 배경색은 하늘색</h5>
		<c:forEach var="card" items="${cards}" varStatus="st">
		...
		<h5>9.앞의 사용법2번에 저장한 cards의 내용 중, 국민카드는 파란색 삼성카드는 빨강색, ' 첫번째 카드의 배경색은 노랑 마지막카드의 배경색은 하늘색</h5>
		<b>숙제2...</b>
