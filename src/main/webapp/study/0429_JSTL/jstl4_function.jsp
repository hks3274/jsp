...
%>
	...
	<div>
		...
		3-2.문자열추출(substring()) : <%=str.substring(5)%> /
															<%-- ${fn:substring()(str, 5)}<br/> --%>
															${fn:substring(str, 5, -1)} / ${fn:substring(str,5,fn:length(str))}<br/>
		<hr/>
		4-1.특정문자의 위치(indexOf)	: <%=str.indexOf("o") %> / ${fn:indexOf(str,'o')}<br/>												
		4-2.특정문자의 위치(lastIndexOf)	: <%=str.lastIndexOf("o") %> /<%--  ${fn:lastIndexOf(str,'o')}<br/> --%>	<br/>
    ...
    마지막 'o'문자의 위치? <font color='red'><b>${(fn:length(str)-1) - (fn:length(strTemps[strTempsLength-1]))}</b></font>
		...
		6.문자열분리(split(변수, 분리할문자)) : 문자열분리후에는 분리된 문자들이 모두 변수에 담아줘야한다. 즉, 이번수는 배열(?)이 된다.<br/>
