...
insert into board values (default, 'admin','관리맨','게시판 서비스를 시작합니다','즐거운 게시판 생활이 되세요', default, '192.168.50.51', default,default,default);

select * from board;
select * from board where idx = 4; /*현재글*/
select idx,title from board where idx > 4 order by idx limit 1; /*다음글*/
select idx,title from board where idx < 4 order by idx desc limit 1; /*이전글*/
