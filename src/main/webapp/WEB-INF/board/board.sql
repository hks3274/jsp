...
insert into boardReply values (default, 24, 'hkd1234', '홍장군', default, '192.168.50.51', '정말 재밌어요^^');
insert into boardReply values (default, 25, 'dtom1234', '디톰', default, '192.168.50.51', '허허 재밌구먼');
insert into boardReply values (default, 20, 'ctom1234', '씨톰맨', default, '192.168.50.51', '멋진글입니다...');
...
select *,(select count(*) from boardReply where boardIdx = 24) as replyCnt from board where idx=24;
select *,(select count(*) from boardReply where boardIdx = b.idx) as replyCnt from board b where idx=24;
