...
insert into complaint values(default, 'board', 20,'hkd1234','광고',default)
...
create table review(
	...
	mid 			varchar(20) not null, 				/* 리뷰 올린이 아이디 */
...
select * from review where partIdx = 12;

select * from reviewReply order by replyIdx desc; 

select r.*, v.partidx from reviewReply r, review v where v.idx = r.ReviewIdx and v.partIdx = 12 order by r.ReviewIdx desc, r.replyIdx desc;
select r.*, v.partidx from review v left join reviewReply r on v.idx = r.ReviewIdx where v.partIdx = 12 and part = 'pds' order by r.ReviewIdx desc, r.replyIdx desc;
select * from (select * from review where partIdx = 12) as v left join reviewReply r on v.idx = r.ReviewIdx order by r.ReviewIdx desc, r.replyIdx desc;
