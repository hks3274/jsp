		...
		function hUpdate() {
			...
			$.ajax({
				...
				success : function(res) {
					if(res != "0") {
						alert("회원 정보 수정 완료!!!");
						/* location.reload(); //전체화면 리로드*/
						//부분 리로드
						$("#heowonList").load(location.href= ' #heowonList');
					}
					else {
						alert("수정 실패..");
						$("#pwd").focus();
		...
		<table class = "table table-bordered">
			...
			<tr>
				<td colspan ="2" class ="text-center">
					...
					<input type="button" value = "회원수정" onclick="hUpdate()" class ="btn btn-secondary mr-2"/>
					<input type="button" value = "전체보기" onclick="location.href='heowonList.st'" class ="btn btn-info mr-2"/>
				</td>
			</tr>
		</table>
	</form>
