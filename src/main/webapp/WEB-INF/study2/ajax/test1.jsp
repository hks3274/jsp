...
<head>
	...
	<script>
		...
		//비동기식 처리 (브라우저에서 제공하는 객체XMLHttpRequest) 사용)
		function idCheck1() {
			let mid = myform.mid.value.trim();
			let flag = idCheck(mid);
...
<div class = "container">
	<div>
		<form name ="myform">
			...
			<div>
				<input type ="button" value ="아이디검색(동기식)" onclick="idCheck0()" class="btn btn-success mt-3 mr-3"/>
				<input type ="button" value ="아이디검색(비동기식1)" onclick="idCheck1()" class="btn btn-primary mt-3 mr-3"/>
				<input type ="button" value ="아이디검색(비동기식2)" onclick="idCheck2()" class="btn btn-info mt-3 mr-3"/>
				<input type ="button" value ="아이디검색(비동기식3)" onclick="idCheck3()" class="btn btn-secondary mt-3 mr-3"/>
				<input type ="button" value ="아이디검색(AJAX1)" onclick="idCheck4()" class="btn btn-warning mt-3 mr-3"/>
				<input type ="button" value ="아이디검색(AJAX2)" onclick="idCheck5()" class="btn btn-danger mt-3 mr-3"/>
			</div>			
		</form>
		<hr/>
	...
	<h2>HTTP통신</h2>
	<pre>
		☞ 동기식(Synchronous) : 먼저 시작된 하나의 작업이 끝날때까지 다른 작업들은 시작하지 않고 기다렸다가, 앞의 작업이 모두 끝나면, 새로운 작업을 시작하는 방식이다.
		...
		☞ AJAX(Asynchronous JavaScript And Xml)
			자바스크립트 라이브러리 중의 하나이며, 브라우저객체인 XMLHttpRequest를 이용해서 전체페이지를 고치지 않아도 부분적인 페이지 일부만을 위한 데이터를 로드하는 기법이다.
		...
		- POST : 새로운 리소그를 생성할때 사용
		- PUT : 리소스를 생성/업데이트 할때 사용
